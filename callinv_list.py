...
from inv_listui import Ui_inv_list
from connectdb import database
from openpyxl import load_workbook
from PyQt5.QtCore import Qt


class callinv_list(QMainWindow, Ui_inv_list):
    ...
    def __init__(self, list_info, parent=None):
        ...
        self.setWindowTitle('INVIOCE NOTE')
    ...
    def da_up_bu(self):
        reply = QMessageBox.information(self, "UPLOAD INVOICE", "Once UPLOAD , Invoice cannot be recovered!",
                                        QMessageBox.Yes | QMessageBox.Close, QMessageBox.Close)
        if reply == QMessageBox.Yes:
            da = self.da_li.text()
            dua = self.dued_li.text()
            inv = self.in_li.text()
            self.condb.up_data(da,dua,inv)
            self.close()
